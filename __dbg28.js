...
for(const step of [1, 2]){
  ...
  for(let i=0;i<9000;i++){
    ...
    if(le+re<3){ degCount++;
      ...
      if(rx<0){ console.log(`step=${step}: NO ROAD i=${i} pts=${points.length} deg=${degCount}`); break; }
    ...
    if(travelled>4500 && d<step*6){ console.log(`step=${step}: CLOSED i=${i} trav=${travelled.toFixed(0)} dist=${d.toFixed(1)} pts=${points.length} deg=${degCount}`); break; }
