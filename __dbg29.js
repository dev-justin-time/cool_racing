...
const step=Math.max(2,Math.round(ratio*12)); // 2px
const spacing=step/ratio;
const points=[];
let travelled=0;
for(let i=0;i<9000;i++){
  const nx=-hz,nz=hx;
  // UNBOUNDED max-DT: scan a fixed radius around the point, pick max dist
  let bx=x,bz=z,bd=-1;
  const R = 68; // the original search radius
  for(let s=-R;s<=R;s++){
  ...
  if(i%500===0) console.log("i",i,"px",x.toFixed(0),z.toFixed(0),"trav",travelled.toFixed(0));
}
console.log("NO CLOSURE pts",points.length,"trav",travelled.toFixed(0));
