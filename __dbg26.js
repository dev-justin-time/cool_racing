...
// walk with DT recenter (exact generator logic)
const ratio=mw/6000;
...
const points=[];
let travelled=0, degCount=0, shiftCount=0;
for(let i=0;i<9000;i++){
  ...
  if(le+re<3){ degCount++;
    ...
    if(rx<0){ console.log("NO ROAD i=",i,"at",x,z,"pts",points.length,"deg",degCount); break; }
  ...
  if(!isRoad(x+hx*step,z+hz*step)){
    ...
    if(!found){ console.log("DEAD END i=",i,"at",x,z,"pts",points.length,"deg",degCount); break; }
  }
  x+=hx*step; z+=hz*step;
  travelled+=spacing;
  const d=Math.hypot(x-startX,z-startZ);
  if(travelled>4500 && d<step*6){ console.log("CLOSED i=",i,"trav",travelled.toFixed(0),"dist",d.toFixed(1),"pts",points.length,"deg",degCount,"shift%",(100*shiftCount/i).toFixed(0)); process.exit(0); }
  if(i%200===0) console.log("i",i,"px",x.toFixed(0),z.toFixed(0),"trav",travelled.toFixed(0),"deg",degCount);
}
console.log("NO CLOSURE pts",points.length,"trav",travelled.toFixed(0),"deg",degCount,"shift%",(100*shiftCount/9000).toFixed(0));
