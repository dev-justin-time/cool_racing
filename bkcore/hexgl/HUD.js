...
{
	this.time = this.formatRaceTime(time.m*60000 + time.s*1000 + time.ms);
}

bkcore.hexgl.HUD.prototype.formatRaceTime = function(milliseconds)
{
	var t = Math.max(0, Math.round(milliseconds || 0));
...
bkcore.hexgl.HUD.prototype.updateLapTime = function(lapMilliseconds, bestMilliseconds)
{
	this.lapTime = this.formatRaceTime(lapMilliseconds);
	this.sessionBest = bestMilliseconds == null ? "—:—.———" : this.formatRaceTime(bestMilliseconds);
}
...
bkcore.hexgl.HUD.prototype.update = function(speed, speedRatio, shield, shieldRatio)
{
	...
	else if(this.step == 1)
	{
		...
		if(this.lapTime != "")
		{
			this.ctx.font = (SCREEN_WIDTH/30)+"px "+this.font;
			this.ctx.fillStyle = "rgba(111, 247, 238, 0.92)";
			this.ctx.fillText("LAP  " + this.lapTime, SCREEN_HW, SCREEN_WIDTH/15);
			this.ctx.font = (SCREEN_WIDTH/52)+"px "+this.font;
			this.ctx.fillStyle = "rgba(233, 245, 244, 0.52)";
			this.ctx.fillText("SESSION BEST  " + this.sessionBest, SCREEN_HW, SCREEN_WIDTH/9);
		}
