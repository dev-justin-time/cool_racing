...
{
	if(time !== 0 && time - this.lastSampleTime < this.sampleInterval) return;
...
bkcore.hexgl.RaceData.prototype.applyInterpolated = function(time)
{
	...
	if(this.seek == this.last || this.seek == 0)
	{
		...
		// Final sample reached: there is no next sample to interpolate with.
		// The caller (modes.replay) ends the run once seek == last.
		if(this.seek == this.last) return;
	}
