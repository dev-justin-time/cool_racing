...
// Restarts the requestAnimationFrame loop after a pause (see HexGL.pause).
// Callers must reset this.manager.time and the gameplay timer so the first
// frame after resume doesn't compute a giant delta.
...
bkcore.hexgl.HexGL.prototype.resize = function(w, h)
{
	...
	var sky = this.manager != null ? this.manager.get('sky') : null;
	if(sky != null && sky.camera != null)
	{
		sky.camera.aspect = rw / rh;
		sky.camera.updateProjectionMatrix();
	}

	if(this.hud != null)
		this.hud.resize(rw, rh);

	if(this.composers.game != null)
	{
		// EffectComposer.reset() re-binds the render targets and resizes the
		// shared fullscreen quad/camera to the current window dimensions.
		// Reuse one render target across resizes when the size is unchanged
...
bkcore.hexgl.HexGL.prototype.initGameplay = function()
{
	...
	this.gameplay = new bkcore.hexgl.Gameplay({
		...
		track: this.track,
			...
			onLapComplete: function(detail) {
				if(window.dispatchEvent) window.dispatchEvent(new CustomEvent('hexgl:lap', { detail: detail }));
			}
		});
	...
	if(this.gameplay.result == this.gameplay.results.FINISH)
	{
		...
		if(typeof(Storage)!=="undefined")
		{
			if(localStorage['score-'+t+'-'+d] == undefined || localStorage['score-'+t+'-'+d] > f)
			{
				...
				// Export race data
				localStorage['race-'+t.name+'-replay'] = JSON.stringify(this.gameplay.raceData.export());
			}
			else
			{
