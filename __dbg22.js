...
function chf(px,py,fwd,out){
  ...
  if(fwd){
    ...
    if(py<mw-1&&px<mw-1) out[k++]=[1,1,4];
    if(py<mw-1) out[k++]=[1,0,3];
    if(py<mw-1&&px>0) out[k++]=[1,-1,4];
    if(px<mw-1) out[k++]=[0,1,3];
  }
  return k;
}
for(let py=0;py<mw;py++) for(let px=0;px<mw;px++){
  const i=py*mw+px; if(!roadMask[i]) continue;
  let best=INF; const k=chf(px,py,true,nb);
  for(let n=0;n<k;n++){ const dv=dist[(py+nb[n][0])*mw+(px+nb[n][1])]; if(dv>=INF) continue; const v=dv+nb[n][2]; if(v<best) best=v; }
  dist[i]=best;
}
for(let py=mw-1;py>=0;py--) for(let px=mw-1;px>=0;px--){
  const i=py*mw+px; if(!roadMask[i]) continue;
  let best=dist[i]; const k=chf(px,py,false,nb);
  for(let n=0;n<k;n++){ const dv=dist[(py+nb[n][0])*mw+(px+nb[n][1])]; if(dv>=INF) continue; const v=dv+nb[n][2]; if(v<best) best=v; }
  dist[i]=best;
}
const dAt=(px,pz)=>{ if(px<0||pz<0||px>=mw||pz>=mw) return 0; return dist[pz*mw+px]; };

// walk (exact generator logic)
const ratio = mw/6000;
...
for(let i=0;i<9000;i++){
  ...
  if(le+re<3){
    ...
    if(rx<0){ console.log("NO ROAD i=",i,"at",x,z); break; }
  ...
  if(!isRoad(x+hx*step,z+hz*step)){
    ...
    if(!found){ console.log("DEAD END i=",i,"at",x,z,"pts",points.length); break; }
  }
  x+=hx*step; z+=hz*step;
  travelled+=spacing;
  const d=Math.hypot(x-startX,z-startZ);
  if(d<minD) minD=d;
  if(travelled>4500 && d<step*6){ console.log("CLOSED i=",i,"trav",travelled.toFixed(0),"dist",d.toFixed(1),"pts",points.length); process.exit(0); }
  if(i%300===0) console.log("i",i,"px",x.toFixed(0),z.toFixed(0),"trav",travelled.toFixed(0),"minD",minD.toFixed(1));
}
console.log("NO CLOSURE pts",points.length,"trav",travelled.toFixed(0),"minD",minD.toFixed(1));
