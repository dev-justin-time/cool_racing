...
for(const TH of [0.5, 0.4, 0.3]){
  ...
  for(let i=0;i<9000;i++){
    ...
    if(le+re<3){ deg++;
      ...
      if(rx<0){ console.log(`th=${TH}: NO ROAD i=${i}`); break; }
    ...
    if(!isRoad(x+hx*step,z+hz*step)){
      ...
      if(!found){ console.log(`th=${TH}: DEAD END i=${i} at ${x},${z} pts=${points.length} deg=${deg}`); break; }
    }
    x+=hx*step; z+=hz*step;
    travelled+=spacing;
    const d=Math.hypot(x-startX,z-startZ);
    if(travelled>4500 && d<step*6){ console.log(`th=${TH}: CLOSED i=${i} trav=${travelled.toFixed(0)} dist=${d.toFixed(1)} pts=${points.length} deg=${deg}`); break; }
    if(i===8999) console.log(`th=${TH}: NO CLOSURE pts=${points.length} trav=${travelled.toFixed(0)} deg=${deg}`);
  }
}
